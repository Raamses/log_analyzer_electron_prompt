...
export function inferSchema(rawColumns: string[]): SchemaEntry {
  // A minimal, generic schema that accepts everything as `unknown`.
  return {
    id: 'generic-inferred',
    label: 'Generic (inferred)',
    timezone: 'utc',
    bindings: Object.fromEntries(
      rawColumns.map(c => [c, { role: inferRole(c), type: 'string' as CellType }]),
    ),
  };
}
